...
namespace 調劑台管理系統
{
    public partial class Form1 : Form
    {
        ...
        void sub_Program_領藥台_01_檢查輸入資料()
        {
            ...
            if (cnt_Program_領藥台_01_檢查輸入資料 == 4) cnt_Program_領藥台_01_檢查輸入資料_檢查醫囑資料及寫入(ref cnt_Program_領藥台_01_檢查輸入資料);
            if (cnt_Program_領藥台_01_檢查輸入資料 == 5) cnt_Program_領藥台_01_檢查輸入資料 = 65500;
        ...
        #region Function
        private bool Function_領藥台_01_檢查重複領藥(List<object[]> list_醫囑資料)
        {
            if (list_醫囑資料.Count == 0) return false;
            List<object[]> list_堆疊母資料 = this.sqL_DataGridView_取藥堆疊母資料.SQL_GetAllRows(false);
            string 藥袋序號 = list_醫囑資料[0][(int)enum_醫囑資料.藥袋條碼].ObjectToString();
            list_堆疊母資料 = list_堆疊母資料.GetRows((int)enum_取藥堆疊母資料.藥袋序號, 藥袋序號);
            if (list_堆疊母資料.Count > 0) return true;
            return false;
        }
        private void Function_領藥台_01_醫囑領藥(string BarCode)
        {
            ...
            daynum = 999;
            daynum *= -1;
            string jsonString = Basic.Net.WEBApiGet($"https://10.97.241.193:4433/api/UDSDBBAR?BarCode={BarCode}");
            ...
            pLC_Device = new PLC_Device(plC_CheckBox_已過帳可再取藥.寫入元件位置);
            ...
            if (list_醫囑資料.Count == 0)
            {
                this.voice.SpeakOnTask("未建立儲位");
                return;
            }
            if (this.Function_領藥台_01_檢查重複領藥(list_醫囑資料))
            {
                this.voice.SpeakOnTask("此藥單領取中");
                return;
            }
            ...
            pLC_Device = new PLC_Device(plC_Button_多醫囑模式.寫入元件位置);
            if (pLC_Device.Bool == false) this.Function_取藥堆疊資料_刪除指定調劑台名稱母資料(this.textBox_工程模式_領藥台_01_名稱.Text);
        ...
        private void Function_領藥台_01_醫囑退藥(string BarCode)
        {
            ...
            list_醫囑資料 = list_醫囑資料.GetRowsInDate((int)enum_醫囑資料.開方日期, dateTime_start, dateTime_end);
            List<object[]> list_醫囑資料_remove = new List<object[]>();
            // if (plC_CheckBox_已過帳可再取藥.Bool == false) list_醫囑資料 = list_醫囑資料.GetRows((int)enum_醫囑資料.狀態, enum_醫囑資料_狀態.已過帳.GetEnumName());
            for (int i = 0; i < list_醫囑資料.Count; i++)
            {
        ...
        void sub_Program_領藥台_02_檢查輸入資料()
        {
            ...
            if (cnt_Program_領藥台_02_檢查輸入資料 == 4) cnt_Program_領藥台_02_檢查輸入資料_檢查醫囑資料及寫入(ref cnt_Program_領藥台_02_檢查輸入資料);
            if (cnt_Program_領藥台_02_檢查輸入資料 == 5) cnt_Program_領藥台_02_檢查輸入資料 = 65500;
        ...
        #region Function
        private bool Function_領藥台_02_檢查重複領藥(List<object[]> list_醫囑資料)
        {
            if (list_醫囑資料.Count == 0) return false;
            List<object[]> list_堆疊母資料 = this.sqL_DataGridView_取藥堆疊母資料.SQL_GetAllRows(false);
            string 藥袋序號 = list_醫囑資料[0][(int)enum_醫囑資料.藥袋條碼].ObjectToString();
        ...
        private void Function_領藥台_02_醫囑領藥(string BarCode)
        {
            ...
            if (this.Function_領藥台_02_檢查重複領藥(list_醫囑資料))
            {
                this.voice.SpeakOnTask("此藥單領取中");
                return;
            }
            ...
            Console.Write($"取得藥品資料 , 耗時{myTimer.ToString()}\n");
            pLC_Device = new PLC_Device(plC_Button_多醫囑模式.寫入元件位置);
            if (pLC_Device.Bool == false) this.Function_取藥堆疊資料_刪除指定調劑台名稱母資料(this.textBox_工程模式_領藥台_02_名稱.Text);
            Console.Write($"刪除調劑台資料資料 , 耗時{myTimer.ToString()}\n");
            for (int i = 0; i < list_醫囑資料.Count; i++)
            {
