    ...
    public partial class Form1 : Form
    {
        ...
        void cnt_Program_取藥堆疊資料_檢查資料_設定致能(ref int cnt)
        {
            ...
            foreach (object[] 取藥堆疊資料 in list_取藥堆疊子資料)
            {
                ...
                byte[] ip_bytes = IP.StrinToIPAddress().GetAddressBytes();
                int ip_value = ip_bytes[2] * 100 + ip_bytes[3];
                if(ip_value >= 4030 && ip_value <= 4079)
                {
                    Voice voice = new Voice();
                    voice.SpeakOnTask("架上左方");                   
                }
                else if (ip_value >= 4130 && ip_value <= 4179)
                {
                    Voice voice = new Voice();
                    voice.SpeakOnTask("架上左方");
                }
                else if (ip_value >= 4230 && ip_value <= 4279)
                {
                    Voice voice = new Voice();
                    voice.SpeakOnTask("架上左方");
                }
                else if (ip_value >= 4330 && ip_value <= 4379)
                {
                    Voice voice = new Voice();
                    voice.SpeakOnTask("架上右方");
                }
                else if (ip_value >= 4430 && ip_value <= 4479)
                {
                    Voice voice = new Voice();
                    voice.SpeakOnTask("架上右方");
                }
                else if (ip_value >= 4530 && ip_value <= 4579)
                {
                    Voice voice = new Voice();
                    voice.SpeakOnTask("架上右方");
                }
                ...
                if (取藥堆疊資料[(int)enum_取藥堆疊子資料.TYPE].ObjectToString() == DeviceType.EPD583_lock.GetEnumName() || 取藥堆疊資料[(int)enum_取藥堆疊子資料.TYPE].ObjectToString() == DeviceType.EPD583.GetEnumName())
                {
